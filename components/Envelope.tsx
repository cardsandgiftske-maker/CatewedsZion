...
export const Envelope: React.FC<EnvelopeProps> = ({ onOpen, isOpened }) => {
  return (
    <div className="relative w-72 h-48 sm:w-96 sm:h-64 cursor-pointer group" onClick={onOpen}>
      <div className="absolute inset-0 bg-[var(--cream-soft)] rounded shadow-2xl border-2 border-[var(--olive-light)] transition-transform duration-1000">
        <div className="absolute inset-0 opacity-10 pointer-events-none" 
             style={{ backgroundImage: `url('https://www.transparenttextures.com/patterns/floral-paper.png')` }}></div>
      </div>

      <div 
        className={`absolute top-0 left-0 w-full h-full bg-[var(--cream-rich)] origin-top transition-all duration-1000 ease-in-out z-20 
      ...
      >
        {/* Olive seal */}
        <div className="relative w-16 h-16 sm:w-20 sm:h-20 bg-[var(--olive-dark)] rounded-full shadow-xl flex items-center justify-center border-4 border-[var(--olive-medium)]">
           <span className="font-serif italic text-white text-xl sm:text-2xl select-none">CZ</span>
